...
export default function CheckoutPage() {
  ...
  const total = useMemo(() => {
    ...
    return (pricing.subtotal || 0) + ship;
        ...
        <div className="min-h-screen bg-gray-50 pt-30">
          <div className="max-w-3xl mx-auto px-4 py-8">
            <div className="bg-white rounded-lg shadow-sm p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">We’ll email your receipt shortly.</p>
              <Link href="/products" className="inline-flex items-center bg-orange-500 text-white px-6 py-3 rounded-lg hover:bg-orange-600">
                Continue Shopping
              </Link>
            </div>
  ...
  const onChange = (e) => {
    ...
    if (name === 'country') {
      ...
      setFormData(p => ({ ...p, country: value, countryCode: c?.code || '', state: '', city: '' }));
            ...
            <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 sticky top-20">
              ...
              <div className="space-y-3 mb-6">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{pricingLoading ? '…' : `$${(pricing.subtotal || 0).toFixed(2)}`}</span>
                </div>
              ...
              <PayPalButtons
                ...
                createOrder={async () => {
                  ...
                  const data = await res.json();
                  if (!res.ok || !data?.id) throw new Error(data?.error || "PayPal create failed");
                ...
                onApprove={async (data) => {
                  ...
                  try {
                    ...
                    if (!capRes.ok || !cap?.ok) throw new Error(cap?.error || "Capture failed");
                    ...
                    if (!orderRes.ok) {
                      ...
                      throw new Error(e?.error || "Failed to create Printify order");
                    }
                    ...
                    console.error(err);
                    alert("Payment captured but order processing failed. We’ll contact you, or reach out to support with your PayPal order ID.");
                  } finally {
              ...
              {(!isFormValid || pricingLoading || isShippingLoading) && (
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                  {!isFormValid ? "Complete required fields and wait for pricing/shipping." :
                    pricingLoading ? "Fetching prices…" :
                    isShippingLoading ? "Calculating shipping…" : ""}
                </div>
              )}
