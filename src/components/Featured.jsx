...
const FeaturedSection = () => {
  ...
  const handleScroll = (dir) => {
    if (scrollRef.current) {
      ...
      const getScrollDistance = () => {
        if (window.innerWidth >= 1536) return 288 + 48; // 2xl
        if (window.innerWidth >= 1280) return 256 + 32; // xl
        if (window.innerWidth >= 1024) return 224 + 24; // lg
        if (window.innerWidth >= 768) return 192 + 16; // md
        return 160 + 16; // sm and below
      };
  ...
  const updateScrollProgress = () => {
    if (scrollRef.current) {
      ...
      const progress = maxScroll > 0 ? Math.min((scrollLeft / maxScroll) * 100, 100) : 0;
  ...
  useEffect(() => {
    ...
    if (scrollElement && !loading) {
      ...
      return () => scrollElement.removeEventListener('scroll', updateScrollProgress);
    }
  }, [loading, featuredProducts]);

  return (
    <div className="relative bg-white overflow-hidden 2xl:py-16 xl:py-14 lg:py-12 md:py-10 py-8 2xl:px-8 xl:px-6 lg:px-4 px-2">

      <div className="relative z-10 max-w-7xl mx-auto">
        {/* Header Section */}
        <div className="2xl:mb-8 xl:mb-7 lg:mb-6 md:mb-5 mb-4">
          <div className="flex lg:flex-row flex-col lg:justify-between lg:items-start items-center 2xl:mb-6 xl:mb-5 lg:mb-4 md:mb-4 mb-3">
            <div className="lg:mb-0 mb-4">
              <h3 className={`${inter.className} 2xl:text-3xl xl:text-2xl lg:text-xl md:text-lg sm:text-base text-sm font-semibold text-gray-900 leading-tight lg:text-left text-center`}>
              The Fan Favorites <br/>Collection.
              </h3>
            </div>
            ...
            <div className="flex flex-col space-y-2 2xl:max-w-xs xl:max-w-sm lg:max-w-xs md:max-w-sm max-w-full">
              ...
              <p className={`${poppins.className} text-gray-500 2xl:text-base xl:text-sm lg:text-xs md:text-xs sm:text-xs text-xs lg:text-left text-center leading-relaxed`}>
              From viral hits to timeless classics, <br/>these are the tees and mugs making everyone smile.
              </p>
            </div>
          </div>
          
          {/* Line indicator with arrows */}
          <div className="flex items-center 2xl:space-x-4 xl:space-x-3 lg:space-x-2 space-x-2">
            ...
            <div className="flex space-x-1">
              ...
              >
                <svg className="2xl:w-4 2xl:h-4 xl:w-3.5 xl:h-3.5 lg:w-3 lg:h-3 md:w-3 md:h-3 w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="relative">
          {loading ? (
            <div className="flex overflow-x-auto 2xl:space-x-4 xl:space-x-3 lg:space-x-2 space-x-2 2xl:px-12 xl:px-8 lg:px-6 md:px-4 px-2 2xl:pb-26 xl:pb-20 lg:pb-16 md:pb-12 pb-8">
              {[...Array(6)].map((_, index) => (
                <div
                  key={index}
              ...
              {featuredProducts.map((product, index) => (
                <Link key={product.id} href={`/products/${product.id}`}>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <h4 className="text-gray-900 font-medium 2xl:text-base xl:text-sm lg:text-xs md:text-xs sm:text-xs text-xs leading-snug">
                        {product.title}
                      </h4>
                      <p className="text-orange-500 font-bold 2xl:text-lg xl:text-base lg:text-sm md:text-sm sm:text-xs text-xs">${product.price}</p>
                    </div>
                  </div>
                </Link>
              ))}
